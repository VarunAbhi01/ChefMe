...
Data fetching is generally is slow process which cant be seen by human eye but yes it is as its a synchronous process. So we need to setup a loading.
...
If theres no data getting back which might be due to wrong API or can any then we need to handle that case...
OR if the search input does not match any item in API then also no data will be returned...
...
#### ######################################## WORKING OF searchterm , text state , handlesubmit nd handlesubmit
 ```
 create text and settext state function.
 set up 1.handlechange and 2.handlesubmit functions.
 assign onchange to 1 in text input line and 2 to onsubmit in entire form line
 along 1 assigning do assgn text to value.
 in 1 grasp text from event.target.value and assgn to settext so whatever u type goes to sesttext.
 in 2 just put prevent default action of submit that reloads the page.

 create search term state function in context
 combine allmealsurl to search term and then send it to fetch data with searchterm as depenendency.
 pass setsearch term to seach component
 there in 2 assign text to setsearch term. so before submitting if theres any text it will be assigned to set search term 
 nd this set search term assigns the serach term which will be sent to fetch data nd thats how we get the data
 ```
...
#### ############################################################################################ Fix Bugs
till now if we frst search for an item it will be displayed correctly but then if you click on surprise u will get a random meal as given fucntino to onclick thats also good buttttttttttt
now if you again type the search again with the previous search item in it it wont show us that req item because that item is already stored in search item nd without it changing in fetch meals function which is a depenedency the fetch meals wont be rendering again so to handle this case we call other function in which we clear the prev search item frst nd then go on with passing radnom url ... 
/components/Search.jsx
```js
import { useState } from 'react'
import { useGlobalContext } from '../context'
...
here selectmeal function is passed to meals from which we will get the idMeal upon clicking any item  and passed through this function. now in selectmeal func we will check which meal id matches to the passed id nd will set selectedmeal to that meal. along that showmodal is set to true so it opens,
